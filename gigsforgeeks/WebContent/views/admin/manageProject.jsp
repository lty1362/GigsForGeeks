        ...
        <div id="content">
        	...
        	<div id="content_r">
		        ...
		        <div class="pagingArea" align="center">
					<% if(currentPage != 1){ %>
			            <!-- 맨 처음으로 (<<) -->
			            <button class="btn btn-outline-info" onclick="location.href='${contextPath}/projectList.bo?currentPage=1';"> &lt;&lt; </button>
			            <!-- 이전페이지로 (<) -->
			            <button class="btn btn-outline-info" onclick="location.href='${contextPath}/projectList.bo?currentPage=<%=currentPage-1%>';"> &lt; </button>
					<% } %>
					
					<% for(int p=startPage; p<=endPage; p++){ %>
						<% if(p != currentPage){ %>
		            	<button class="btn btn-outline-info" onclick="location.href='${contextPath}/projectList.bo?currentPage=<%=p%>';"><%= p %></button>
		            	<% }else{ %>
					...
					<% if(currentPage != maxPage){ %>
			            ...
			            <button class="btn btn-outline-info" onclick="location.href='${contextPath}/projectList.bo?currentPage=<%=currentPage+1%>';"> &gt; </button>
			            <!-- 맨 끝으로 (>>) -->
			            <button class="btn btn-outline-info" onclick="location.href='${contextPath}/projectList.bo?currentPage=<%=maxPage%>';"> &gt;&gt; </button>
					<% } %>
          	 	</div>
